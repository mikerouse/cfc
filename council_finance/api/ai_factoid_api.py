...
class AIFactoidRateThrottle(UserRateThrottle):
    ...
    Rate: 2 requests per hour per council (further reduced to prevent excessive API usage).
    """
    scope = 'ai_factoids'
    rate = '2/hour'
...
def ai_council_factoids(request, council_slug):
    ...
    try:
        ...
        generation_lock_key = f"ai_generation_lock:{council_slug}"
        ...
        # Check if another request is already generating factoids for this council
        generation_in_progress = cache.get(generation_lock_key)
        if generation_in_progress:
            logger.info(f"🔄 Generation already in progress for {council.name} - serving stale/cached data")
            if stale_cached_response:
                stale_cached_response['cache_status'] = 'generation_in_progress_fallback'
                stale_cached_response['served_at'] = timezone.now().isoformat()
                return Response(stale_cached_response, status=status.HTTP_200_OK)
            
            # No stale cache, return wait message
            return Response({
                'success': True,
                'no_factoids': True,
                'message': f'AI analysis for {council.name} is currently being generated by another request. Please try again in a few moments.',
                'council': council_slug,
                'factoids': [],
                'cache_status': 'generation_in_progress',
                'retry_after': 30
        ...
        # Set generation lock to prevent concurrent requests
        cache.set(generation_lock_key, True, 300)  # 5-minute lock
        
        try:
            # Try to generate fresh AI factoids with reduced retry attempts
            factoids = None
            generation_success = False
            retry_count = 0
            max_retries = 1  # Reduced from 3 to 1 to minimize API calls
            ...
            while retry_count < max_retries and not generation_success:
                try:
                    ...
                    if retry_count < max_retries:
                        # Wait longer before retrying to reduce API pressure
                        import time
            ...
            # Enhanced caching strategy - significantly longer cache to reduce API calls:
            # - Primary cache: 30 days (2592000 seconds) - much longer cache
            # - Stale cache: 90 days (7776000 seconds) - for fallback when AI service unavailable
            cache.set(cache_key, response_data, 2592000)  # 30 days primary cache
            cache.set(cache_key_stale, response_data, 7776000)  # 90 days stale cache for fallback
