...
def home(request):
    ...
    # RACE CONDITION FIX: Use cache locking to prevent multiple simultaneous
    # expensive calculations when cache is cold. This prevents the homepage
    # from triggering multiple concurrent SiteTotalsAgent runs during high traffic.
    cache_check_lock_key = "homepage_cache_check_lock"
    
    # Try to acquire a lock for cache checking - prevents race conditions
    from django.core.cache import cache
    cache_check_in_progress = cache.get(cache_check_lock_key)
    
    if cache_check_in_progress:
        # Another request is already checking cache, skip expensive detection
        logger.info("Cache check already in progress, using fallback approach")
        missing_cache = False  # Use existing cached values or fallbacks
    else:
        # Acquire lock for 30 seconds while we check cache status
        cache.set(cache_check_lock_key, True, 30)
        ...
        try:
            ...
            logger.error(f"Error during cache detection: {e}")
            missing_cache = False  # Safe fallback to prevent expensive operations
        finally:
            # Release the cache check lock
            cache.delete(cache_check_lock_key)
